...
export const analyzeCampaign = async (
  ...
  // Build compact table: header row + up to 30 data rows
  const headerLine = headers.join(' | ');
  const dataLines = rows.slice(0, 30).map(r =>
    headers.map(h => {
      const v = r[h];
  ...
  const prompt = `
Bạn là chuyên gia phân tích marketing và kinh doanh hàng đầu Việt Nam.
User vừa upload dữ liệu chiến dịch marketing của doanh nghiệp họ. Hãy phân tích toàn diện.

=== CÁC CỘT DỮ LIỆU ===
${headerLine}

=== DỮ LIỆU (${rows.length} dòng, hiển thị 30 dòng đầu) ===
${dataLines}
${rows.length > 30 ? `\n... và ${rows.length - 30} dòng tiếp theo` : ''}

=== YÊU CẦU ===
Không cần biết tên sản phẩm hay ngành hàng — hãy phân tích dựa trên số liệu thực tế có trong bảng.
1. Xác định: cột nào là doanh thu/doanh số? cột nào thể hiện hoạt động marketing?
2. Phân tích xu hướng: tăng trưởng, biến động, mùa vụ
3. Đánh giá tương quan: chi phí marketing / hoạt động → kết quả kinh doanh
4. ROI và hiệu quả đầu tư marketing (nếu có số liệu chi phí)
5. Điểm mạnh, điểm yếu, cơ hội cải thiện
6. Khuyến nghị cụ thể: có nên TIẾP TỤC, ĐIỀU CHỈNH hay DỪNG chiến lược?

Trả về JSON hợp lệ (không kèm markdown):
{
  "analysisText": "Báo cáo 400-600 từ tiếng Việt, trình bày theo từng mục rõ ràng",
  "recommendation": "continue" | "pivot" | "stop",
  "effectivenessScore": <số nguyên 0-100>,
  "dateColumn": "<tên cột ngày/thời gian trong bảng, hoặc null nếu không có>",
  "revenueColumn": "<tên cột doanh thu chính trong bảng, hoặc null>",
  "activityColumn": "<tên cột thể hiện hoạt động marketing (chi phí QC, số bài, v.v.), hoặc null>"
}

effectivenessScore: 0-40 = kém (stop), 41-65 = trung bình (pivot), 66-100 = tốt (continue)
  `.trim();
  ...
  return callWithRetry(async () => {
    ...
    return {
      ...
      revenueColumn:     parsed.revenueColumn ?? null,
      activityColumn:    parsed.activityColumn ?? null,
    };
  });
};
